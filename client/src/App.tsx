...
function Dashboard() {
  ...
  const activeTopic = activeTech?.topics.find((t) => t.id === activeTopicId) ?? null;
  ...
  async function addTopic(techId: string) {
    ...
    setTechnologies((prev) =>
      prev.map((t) => (t.id === techId ? { ...t, topics: [...t.topics, { ...topic, questions: [] }] } : t))
    );
        ...
        t.id !== activeTechId
          ...
          : {
              ...
              topics: t.topics.map((tp) =>
                tp.id !== activeTopic.id ? tp : { ...tp, questions: [...tp.questions, question] }
              ),
            }
      )
    );
          ...
          : {
              ...
              topics: t.topics.map((tp) =>
                tp.id !== activeTopic.id
                  ? tp
                  : { ...tp, questions: tp.questions.map((q) => (q.id === updated.id ? updated : q)) }
              ),
            }
      )
    );
      ...
      prev.map((t) =>
        t.id !== activeTechId
          ...
          : {
              ...t,
              topics: t.topics.map((tp) =>
                tp.id !== activeTopic.id ? tp : { ...tp, questions: tp.questions.filter((q) => q.id !== qid) }
              ),
            }
      )
    );
      ...
      <main className="flex-1 h-screen overflow-y-auto p-8 max-w-3xl">
        {activeTopic && activeTech ? (
          <>
            <div className="no-print flex items-center justify-between mb-6">
              <div>
                ...
                <h2 className="font-serif text-2xl font-semibold">{activeTopic.name}</h2>
              </div>
            ...
            {activeTopic.questions.map((q) => (
              <QnACard key={q.id} question={q} onLocalUpdate={updateQuestionLocal} onDelete={() => deleteQuestion(q.id)} />
            ))}

            <button onClick={addQuestion} className="no-print text-sm text-ink-soft hover:text-deep mt-2">
              + Add question
            </button>
          </>
        ) : (
          <div className="text-ink-soft mt-20 text-center">
            ...
            <p className="text-sm">Add a technology and topic from the sidebar to start.</p>
          </div>
        )}
      </main>

      {printMode && activeTopic && activeTech && (
        <PrintView topic={activeTopic} techName={activeTech.name} mode={printMode} onClose={() => setPrintMode(null)} />
      )}
    </div>
  );
}
